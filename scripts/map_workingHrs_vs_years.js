...
const PlotMap = async function () {
  // Original Source: https://github.com/topojson/world-atlas
  ...
  function updateMap(givenYear) {
    ...
    mapArea
      ...
      .text((d) => "Weekly working hour per worker, " + currentYear)
      .attr("x", 500)
      .attr("y", 40);
  }
  ...
  mapArea
    ...
    .attr("alignment-baseline", "hanging")
    .style("font-size", "25px")
    .text("Weekly working hour per worker, 1980")
    .attr("x", 500)
    .attr("y", 40);
  ...
  // Since cutoffs doesn't include 0 and 100 percentile, add start and end manually
